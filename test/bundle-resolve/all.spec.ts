...
import * as Resolve from '../../lib/bundle-resolve';
import * as Utils from '../../lib/bundle-resolve/utils';
...
function getPromiseForEvents(
	...
	const resolvePromise = new Promise((resolve, reject) => {
		...
		if (resolveOnEnd) {
			// FIXME: remove "as any". this is needed because
			// `listeners` is declared as [() => void] but the
			// promise needs to be resolved with { event: something }
			// to stay compatible.
			listeners['end'] = [resolve as any];
		}
