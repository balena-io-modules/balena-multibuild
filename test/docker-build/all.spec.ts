...
import { Builder, BuildHooks } from '../../lib/docker-build';
import * as Utils from '../../lib/docker-build/utils';

const TEST_FILE_PATH = 'test/docker-build/test-files';
...
describe('Tar stream build', () => {
	...
	it('should fail to build if Utils.extractLayer throws an error', async function () {
		...
		const builderMod = proxyquire('../../lib/docker-build/builder', {
			'./utils': mockUtils,
		});
		return new Bluebird((resolve, reject) => {
			const tarStream = fs.createReadStream(
